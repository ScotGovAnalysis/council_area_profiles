...
require(kableExtra)
require(scales) # use comma separation
...
#paramsarea <- "Aberdeen City"



# Initial values - THESE NEED TO BE CHANGED WHEN UPDATING ================================


# Start and end years for each chapter

# Population estimates
pop_est_start_year <- 1998
pop_est_end_year <- 2018

# Births, deaths, marriages and civil partnerships
bir_dea_marr_est_start_year <- 1998
cp_start_year <- 2005
bir_dea_marr_cp_est_end_year <- 2018

# Household estimates
house_est_start_year <- 2001
house_est_end_year <- 2018

# Dwellings estimates
dwell_est_start_year <- 2001
dwell_est_end_year <- 2017

# Household projections
house_proj_start_year <- 2016
house_proj_end_year <- 2026

# Population projections
pop_proj_start_year <- 2016
pop_proj_end_year <- 2026

# Migration
mig_start_year <- "2001-02"
mig_rate_start_year <- "2008-09"
mig_end_year <- "2017-18"
mig_mid_year <- 2018
mig_penultimate_year <- "2016-17"

# Life expectancy
life_exp_start_year <- "2001-03"
life_exp_end_year <- "2015-17"


# Last and next updates

# Population estimates
pop_est_last_update <- "April 2019"
pop_est_next_update <- "April 2020"

# Population projections
pop_proj_last_update <- "March 2018"
pop_proj_next_update <- "March 2020"

# Births
births_last_update <- "June 2019"
births_next_update <- "June 2020"

# Deaths
deaths_last_update <- "June 2019"
deaths_next_update <- "June 2020"

# Life expectancy
life_exp_last_update <- "December 2018"
life_exp_next_update <- "December 2019"

# Migration
mig_last_update <- "April 2019"
mig_next_update <- "April 2020"

# Marriages and civil partnerships
marr_cp_last_update <- "June 2019"
marr_cp_next_update <- "June 2020"

# Household estimates
house_est_last_update <- "June 2019"
house_est_next_update <- "June 2020"

# Household projections
house_proj_last_update <- "July 2018"
house_proj_next_update <- "July 2020"

# Dwellings
dwell_last_update <- "May 2018"
dwell_next_update <- "September 2019"



# Read dataset ==========================================================================

dataset <- "council-area-profiles-dataset.xlsx"

n <- 22;

for (i in 1:n) {
  print(paste(i, excel_sheets(dataset)[i]))
  assign(excel_sheets(dataset)[i], read_excel(dataset, sheet=i))#[1:nr[i], 1:nc[i]]
}
...
pop_est <- mutate(`population-estimates`,
                  Age_group = cut(Age, c(-1,15,24,44,64,74,130), 
                                  labels=c("0 to 15", "16 to 24", "25 to 44", "45 to 64",
                                           "65 to 74", "75 and over" )),
                  Area = `Council area`,
                  Number = Population) %>%
  mutate(Age_group = factor(Age_group, levels = c("All ages", "0 to 15", "16 to 24", "25 to 44", "45 to 64",
                                                  "65 to 74", "75 and over"))) %>%
...
# and end year.
base_pop_est = rep(filter(total_pop_est, Year == min(Year))$Number, each = pop_est_end_year - pop_est_start_year + 1)

# Use base_pop_est to compute the percentage change from the start year for each council area for each year
total_pop_est <- mutate(total_pop_est, Perc = (Number - base_pop_est)/base_pop_est*100)
...
total_pop_est_end_year <- total_pop_est %>%
  ...
  mutate(Difference = Number - total_pop_est$Number[total_pop_est$Year == (pop_est_end_year-1) & total_pop_est$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) %>%
...
total_pop_est_gender_comp <- pop_est %>%
  group_by(Area, Year, Sex, Age) %>%
  summarise(Number=sum(Number)) %>%
  filter(Year %in% c(pop_est_start_year, pop_est_end_year), Area==paramsarea) %>%
  ungroup()
...
total_pop_est_age_perc <- total_pop_est_age_perc %>%
  mutate(Perc = (total_pop_est_age_perc[[as.character(pop_est_end_year)]] - total_pop_est_age_perc[[as.character(pop_est_start_year)]])/total_pop_est_age_perc[[as.character(pop_est_start_year)]]*100,
...
# and end year.
base_pop_proj = rep(filter(total_pop_proj, Year == min(Year))$Number, each = pop_proj_end_year - pop_proj_start_year + 1)

# Use base_pop_proj to compute the percentage change from the start year for each council area for each year
total_pop_proj <- mutate(total_pop_proj, Perc = (Number - base_pop_proj)/base_pop_proj*100)
...
total_pop_proj_end_year <- total_pop_proj %>%
  ...
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - total_pop_proj$Number[total_pop_proj$Year == pop_proj_start_year & total_pop_proj$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) %>%
  mutate(Rank_change = min_rank(desc(Perc)))
...
total_pop_proj_sex <- pop_proj %>%
  group_by(Area, Year, Sex, Age) %>%
  summarise(Number=sum(Number)) %>%
  filter(Year %in% c(pop_proj_start_year,pop_proj_end_year), Area==paramsarea) %>%
  ungroup()
...
total_pop_proj_age_perc <- total_pop_proj_age_perc %>%
  mutate(Perc = (total_pop_proj_age_perc[[as.character(pop_proj_end_year)]] - total_pop_proj_age_perc[[as.character(pop_proj_start_year)]])/total_pop_proj_age_perc[[as.character(pop_proj_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal"))) %>%
  group_by(Sign) %>% 
...
pop_proj_nature <- pop_proj_nature %>%
  mutate(Natural_perc = (Natural_change/pop_proj_nature[[as.character(pop_proj_start_year)]])*100,
         Mig_perc = (Net_migration/pop_proj_nature[[as.character(pop_proj_start_year)]])*100,
         Total_perc = (Population_change/pop_proj_nature[[as.character(pop_proj_start_year)]])*100)
...
pop_proj_nature_perc_plot <- pop_proj_nature %>%
  ...
  mutate(Type = factor(Type, levels = c("Natural_perc", "Mig_perc", "Total_perc"))) %>%
  arrange(Type) %>%
  group_by(Area) %>%
  filter(Area %in% c(paramsarea, "Scotland")) %>%
  ungroup() %>%
  mutate(Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal")))
...
# use it after we have produced the plots, to re-label the axes with the string values. 
# We define Year to be the last two digits of Year_2, so it is numeric and can be easily used for plots.
net_mig <- mutate(`net-migration`, Area = `Council area`, Age_group = `Age group`, Year_2 = Year) %>%
  select(-Year) %>%
  mutate(Year = as.numeric(substr(Year_2, nchar(Year_2)-1, nchar(Year_2)))) %>%
  mutate(Age_group = factor(Age_group, levels = c("All ages", "0 to 4", "5 to 9", "10 to 14", "15 to 19", "20 to 24", "25 to 29", "30 to 34", "35 to 39", "40 to 44", "45 to 49", "50 to 54", "55 to 59", "60 to 64", "65 to 69", "70 to 74", "75 to 79", "80 to 84", "85 to 89", "90 and over"))) %>%
  arrange(Age_group) %>%
...
# use it after we have produced the plots, to re-label the axes with the string values. 
# We define Year to be the last two digits of Year_2, so it is numeric and can be easily used for plots.
net_mig_rates <- mutate(`net-migration-rates`, Area = `Council area`, Year_2 = Year, Number = Rate) %>%
  select(-Year) %>%
  mutate(Year = as.numeric(substr(Year_2, nchar(Year_2)-1, nchar(Year_2))))
...
net_mig_rates_end_year <- net_mig_rates %>%
  ...
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - net_mig_rates$Number[net_mig_rates$Year_2 == mig_penultimate_year & net_mig_rates$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal")))
...
data_marr <- mutate(`marriages`, Area = `Council area`, Year = `Registration Year`, Number = `Number of marriages`)
...
marr_end_year <- total_marr %>%
  ...
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - total_marr$Number[total_marr$Year == (bir_dea_marr_cp_est_end_year-1) & total_marr$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) #%>%
...
data_cp <- mutate(`civil-partnerships`, Area = `Council area`, Year = `Registration Year`, Number = `Number of civil partnerships`) %>%
...
cp_end_year <- total_cp %>%
  filter(Area != "Scotland", Year == bir_dea_marr_cp_est_end_year) %>%
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - total_cp$Number[total_cp$Year == (bir_dea_marr_cp_est_end_year-1) & total_cp$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) #%>%
...
data_house_est <- mutate(`household-estimates`, Area = `Council area`, Number = `Number of households`)
...
# estimate for each council area once for each year. e.g. repeat 20 times for a 20 year period between start
# and end year.
base_house_est = rep(filter(total_house_est, Year == min(Year))$Number, each = house_est_end_year - house_est_start_year + 1)

# Use base_house_est to compute the percentage change from the start year for each council area for each year
total_house_est <- mutate(total_house_est, Perc = (Number - base_house_est)/base_house_est*100)
...
house_est_end_year_data <- total_house_est %>%
  ...
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - total_house_est$Number[total_house_est$Year == (house_est_end_year-1) & total_house_est$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) %>%
...
data_dwell_num <- mutate(`dwellings`, Area = `Council area`, Number = `Number of dwellings`) %>%
...
# estimate for each council area once for each year. e.g. repeat 20 times for a 20 year period between start
# and end year.
base_dwell_num = rep(filter(total_dwell_num, Year == min(Year))$Number, each = dwell_est_end_year - dwell_est_start_year + 1)

# Use base_dwell_num to compute the percentage change from the start year for each council area for each year
total_dwell_num <- mutate(total_dwell_num, Perc = (Number - base_dwell_num)/base_dwell_num*100)
...
dwell_end_year <- total_dwell_num %>%
  ...
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - total_dwell_num$Number[total_dwell_num$Year == (dwell_est_end_year-1) & total_dwell_num$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) %>%
  mutate(Rank_perc = min_rank(desc(Perc)))
...
data_dwell_type <- data_dwell_type %>%
  filter(Type != "Total") %>%
  mutate(Type = factor(Type, levels = c("Total", "Detached", "Semi-detached", "Terrace", "Flat", "Not known")))
...
data_dwell_type_ca_plot <- data_dwell_type_ca %>%
  mutate(Type = factor(Type, levels = c("Detached", "Semi-detached", "Terrace", "Flat", "Not known")))
...
data_dwell_tax <- mutate(`dwellings-by-council-tax-band`, Area = `Council area`, Type = `Council Tax band`)
...
# projection for each council area once for each year. e.g. repeat 20 times for a 20 year period between start
# and end year.
base_house_proj = rep(filter(total_house_proj, Year == min(Year))$Number, each = house_proj_end_year - house_proj_start_year + 1)

# Use base_house_proj to compute the percentage change from the start year for each council area for each year
total_house_proj <- mutate(total_house_proj, Perc = (Number - base_house_proj)/base_house_proj*100) %>%
...
house_proj_end_year_dataset <- total_house_proj %>%
  ...
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - total_house_proj$Number[total_house_proj$Year == (house_proj_end_year-1) & total_house_proj$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal"))) %>%
  mutate(Rank_perc = min_rank(desc(Perc)))
...
house_proj_age_table <- data_house_proj %>% 
  ...
  ungroup() %>%
  mutate(Age_group = factor(Age_group, levels = c("All ages", "16 to 19", "20 to 24", "25 to 29", "30 to 34", "35 to 39", "40 to 44",
                                                  "45 to 49", "50 to 54", "55 to 59", "60 to 64", "65 to 69", "70 to 74", "75 to 79", "80 to 84", "85 to 89", "90 and over")))
...
house_proj_age_perc <- house_proj_age_perc_spread %>%
  mutate(Perc = (house_proj_age_perc_spread[[as.character(house_proj_end_year)]] - house_proj_age_perc_spread[[as.character(house_proj_start_year)]])/house_proj_age_perc_spread[[as.character(house_proj_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal")))
...
house_proj_type <- data_house_proj %>% 
  ...
  select(Area,Year,`One adult`,`Two adults`,`One adult, one or more children`,`Two or more adults, one or more children`,`Three or more adults`, `All households`) %>%
  gather(key=Type, value=Number, -Area, -Year) %>%
  mutate(Type = factor(Type, levels = c("All households", "One adult", "Two adults", "One adult, one or more children", "Two or more adults, one or more children", "Three or more adults"))) %>%
  arrange(Type) %>%
  mutate(Number = round(Number, 0))
...
house_proj_type_perc <- house_proj_type_perc_prelim %>%
  mutate(Perc = (house_proj_type_perc_prelim[[as.character(house_proj_end_year)]] - house_proj_type_perc_prelim[[as.character(house_proj_start_year)]])/house_proj_type_perc_prelim[[as.character(house_proj_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal"))) %>%
  filter(Area == paramsarea)
...
base_births = rep(filter(total_births, Year == min(Year))$Number, each = bir_dea_marr_cp_est_end_year - bir_dea_marr_est_start_year + 1)

# Use base_births to compute the percentage change from the start year for each council area for each year
total_births <- mutate(total_births, Perc = (Number - base_births)/base_births*100)
...
births_mothers_age <- mutate(`births-by-age-of-mother`,
  ...
  mutate(Age_group = factor(Age_group, levels = c("All ages", "0 to 19", "20 to 24", "25 to 29", "30 to 34", "35 to 39", "40 and over", "Not stated")))
...
total_births_mothers_age_perc <- total_births_mothers_age_perc_prelim %>%
  mutate(Perc = (total_births_mothers_age_perc_prelim[[as.character(bir_dea_marr_cp_est_end_year)]] - total_births_mothers_age_perc_prelim[[as.character(bir_dea_marr_est_start_year)]])/total_births_mothers_age_perc_prelim[[as.character(bir_dea_marr_est_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal")))
...
total_births_mothers_age_perc_scotland <- total_births_mothers_age_perc_scotland_prelim %>%
  mutate(Perc = (total_births_mothers_age_perc_scotland_prelim[[as.character(bir_dea_marr_cp_est_end_year)]] - total_births_mothers_age_perc_scotland_prelim[[as.character(bir_dea_marr_est_start_year)]])/total_births_mothers_age_perc_scotland_prelim[[as.character(bir_dea_marr_est_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal"))) 
...
births_rate_end_year <- births_rate %>%
  filter(Area != "Scotland", Year == bir_dea_marr_cp_est_end_year) %>%
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - births_rate$Number[births_rate$Year == (bir_dea_marr_cp_est_end_year-1) & births_rate$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal")))
...
base_fert_rate = rep(filter(fert_rate, Year == min(Year))$Number, each = bir_dea_marr_cp_est_end_year - bir_dea_marr_est_start_year + 1)

# Use base_fert_rate to compute the percentage change from the start year for each council area for each year
fert_rate <- mutate(fert_rate, Perc = (Number - base_fert_rate)/base_fert_rate*100)
...
fert_rate_end_year <- fert_rate %>%
  filter(Area != "Scotland", Year == bir_dea_marr_cp_est_end_year) %>%
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - fert_rate$Number[fert_rate$Year == (bir_dea_marr_cp_est_end_year-1) & fert_rate$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal")))
...
# council area once for each year. e.g. repeat 20 times for a 20 year period between start and end year.
base_deaths = rep(filter(total_deaths, Year == min(Year))$Number, each = bir_dea_marr_cp_est_end_year - bir_dea_marr_est_start_year + 1)

# Use base_deaths to compute the percentage change from the start year for each council area for each year
total_deaths <- mutate(total_deaths, Perc = (Number - base_deaths)/base_deaths*100)
...
total_deaths_sex_perc <- total_deaths_sex_perc %>%
  mutate(Perc = (total_deaths_sex_perc[[as.character(bir_dea_marr_cp_est_end_year)]] - total_deaths_sex_perc[[as.character(bir_dea_marr_est_start_year)]])/total_deaths_sex_perc[[as.character(bir_dea_marr_est_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal")))
...
total_deaths_sex_perc_scotland <- total_deaths_sex_perc_scotland %>%
  mutate(Perc = (total_deaths_sex_perc_scotland[[as.character(bir_dea_marr_cp_est_end_year)]] - total_deaths_sex_perc_scotland[[as.character(bir_dea_marr_est_start_year)]])/total_deaths_sex_perc_scotland[[as.character(bir_dea_marr_est_start_year)]]*100,
         Sign = ifelse(Perc > 0, "positive", ifelse(Perc < 0, "negative", "equal")))
...
deaths_age_sex <- 
  mutate(`deaths-by-sex-by-age`,
         ...
         Year = `Registration Year`,
         Age_group = `Age group`) %>%
  mutate(Age_group = factor(Age_group, levels = c("All ages", "0", "1 to 4", "5 to 9", "10 to 14", "15 to 19", "20 to 24", "25 to 29", "30 to 34", "35 to 39", "40 to 44", "45 to 49", "50 to 54", "55 to 59", "60 to 64", "65 to 69", "70 to 74", "75 to 79", "80 to 84", "85 to 89", "90 and over"))) %>%
...
deaths_rate_end_year <- deaths_rate %>%
  filter(Area != "Scotland", Year == bir_dea_marr_cp_est_end_year) %>%
  mutate(Rank = min_rank(desc(Number))) %>%
  mutate(Difference = Number - deaths_rate$Number[deaths_rate$Year == (bir_dea_marr_cp_est_end_year-1) & deaths_rate$Area != "Scotland"]) %>%
  mutate(Change_type = ifelse(Difference < 0, "decrease", ifelse(Difference > 0, "increase", "equal")))
...
base_life_exp_from_birth_f = rep(filter(life_exp_from_birth_f_unrounded, Year == min(Year))$Number, each = max(life_exp_from_birth_f_unrounded$Year) - min(life_exp_from_birth_f_unrounded$Year) + 1)

# Use base_life_exp_from_birth_f to compute the percentage change from the start year for each council area for each year
life_exp_from_birth_f_unrounded <- mutate(life_exp_from_birth_f_unrounded, Perc = (Number - base_life_exp_from_birth_f)/abs(base_life_exp_from_birth_f)*100)
...
# year life expectancy for each council area once for each year. e.g. repeat 20 times for a 20 year period between start and end year.
base_life_exp_from_birth_m = rep(filter(life_exp_from_birth_m_unrounded, Year == min(Year))$Number, each = max(life_exp_from_birth_m_unrounded$Year) - min(life_exp_from_birth_m_unrounded$Year) + 1)

# Use base_life_exp_from_birth_m to compute the percentage change from the start year for each council area for each year
life_exp_from_birth_m_unrounded <- mutate(life_exp_from_birth_m_unrounded, Perc = (Number - base_life_exp_from_birth_m)/abs(base_life_exp_from_birth_m)*100)
...
base_life_exp_at_65_f = rep(filter(life_exp_at_65_f_unrounded, Year == min(Year))$Number, each = max(life_exp_at_65_f_unrounded$Year) - min(life_exp_at_65_f_unrounded$Year) + 1)

# Use base_life_exp_at_65_f to compute the percentage change from the start year for each council area for each year
life_exp_at_65_f_unrounded <- mutate(life_exp_at_65_f_unrounded, Perc = (Number - base_life_exp_at_65_f)/abs(base_life_exp_at_65_f)*100)
...
# life expectancy for each council area once for each year. e.g. repeat 20 times for a 20 year period between start and end year.
base_life_exp_at_65_m = rep(filter(life_exp_at_65_m_unrounded, Year == min(Year))$Number, each = max(life_exp_at_65_m_unrounded$Year) - min(life_exp_at_65_m_unrounded$Year) + 1)

# Use base_life_exp_at_65_m to compute the percentage change from the start year for each council area for each year
life_exp_at_65_m_unrounded <- mutate(life_exp_at_65_m_unrounded, Perc = (Number - base_life_exp_at_65_m)/abs(base_life_exp_at_65_m)*100)
